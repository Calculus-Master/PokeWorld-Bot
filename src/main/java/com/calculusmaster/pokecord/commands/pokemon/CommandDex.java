        ...
        if(this.msg.length == 1 || (this.msg.length == 2 && this.isNumeric(1)))
        {
            ...
            for(int i = indices[0]; i < indices[1]; i++)
            {
                ...
                try
                {
                    ...
                    list.append(name).append(d.containsKey(this.player.getId()) && d.getInteger(this.player.getId()) > 0 ? ":white_check_mark:" : ":x:").append(" - Owned: ").append(d.getInteger(this.player.getId())).append("\n");
                }
                catch (Exception e)
                {
