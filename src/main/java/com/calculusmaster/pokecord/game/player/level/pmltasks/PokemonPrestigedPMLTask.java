...
public class PokemonPrestigedPMLTask extends AbstractPMLTask
{
    ...
    {
        return p.getStatistics().get(PlayerStatistic.POKEMON_PRESTIGED) >= this.amount;
    }

    @Override
    public String getProgressOverview(PlayerDataQuery p)
    {
        return p.getStatistics().get(PlayerStatistic.POKEMON_PRESTIGED) + " / " + this.amount + " Pokemon Prestiged";
    }
}
