...
public class PokemonCaughtPMLTask extends AbstractPMLTask
{
    ...
    {
        return p.getStatistics().get(PlayerStatistic.POKEMON_CAUGHT) >= this.amount;
    }

    @Override
    public String getProgressOverview(PlayerDataQuery p)
    {
        return p.getStatistics().get(PlayerStatistic.POKEMON_CAUGHT) + " / " + this.amount + " Pokemon Caught";
    }
}
