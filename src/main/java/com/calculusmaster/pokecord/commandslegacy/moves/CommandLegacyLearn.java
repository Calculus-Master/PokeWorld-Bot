...
public class CommandLegacyLearn extends CommandLegacy
{
    public static Map<String, MoveEntity> moveLearnRequests = new HashMap<>();

    public CommandLegacyLearn(MessageReceivedEvent event, String[] msg)
    {
        super(event, msg);
    }
    ...
    public CommandLegacy runCommand()
    {
        ...
        else
        {
            ...
            if(autoReplace)
            {
                ...
                this.embed = null;
                Executors.newSingleThreadScheduledExecutor().schedule(() -> CommandsLegacy.execute("replace", this.event, new String[]{"replace", "" + this.getInt(1)}), 450, TimeUnit.MILLISECONDS);
            }
        }
